    ...
    ctx.ui.setFooter((tui, theme, footerData: ReadonlyFooterDataProvider) => {
      ...
      return {
        ...
        render(width: number) {
          ...
          const contextTokens =
            model.contextTokens === null ? "?" : formatTokens(model.contextTokens);
          const speed =
            model.tokensPerSecond === null
              ? "— tok/s"
              : `${Math.round(model.tokensPerSecond)} tok/s`;
          const usage = `${contextTokens}/${contextWindow} tokens (${contextPercent}%) · $${model.cost.toFixed(2)} · ${speed}`;

          const changedFiles = `${git.changedFiles} ${git.changedFiles === 1 ? "file" : "files"} changed`;
          const gitLabel = git.isRepository
            ? `${formatGitStatus(theme, git, diff)} ${theme.fg("muted", `· ${changedFiles}`)}`
            : "";

          const lines = [
            columns(location, theme.fg("muted", modelLabel), width),
